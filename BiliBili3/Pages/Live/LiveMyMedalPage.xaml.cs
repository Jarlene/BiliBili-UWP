    ...
    public sealed partial class LiveMyMedalPage : Page
    {
        ...
        private async void Cancel()
        {
            ...
            try
            {
                ...
                string url = string.Format("http://live.bilibili.com/AppUser/canelMedal?access_key={0}&appkey={1}&build=433000&mobi_app=android&platform=wp", ApiHelper.access_key, ApiHelper._appKey);
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
        ...
        private async void Add(int id)
        {
            try
            {
                ...
                string url = string.Format(" http://live.bilibili.com/AppUser/wearMedal?access_key={0}&appkey={1}&build=433000&platform=wp", ApiHelper.access_key, ApiHelper._appKey);
                url += "&sign=" + ApiHelper.GetSign(url);
