...
namespace bilibili2.PartPages
{
    ...
    public sealed partial class PartPage : Page
    {
        ...
        private void GetSetting()
        {
            ...
            {
                switch (int.Parse(settings.GetSettingValue("PartOrderBy").ToString()))
                {
                    case 0:
                        defu_Order = "default";
                        break;
            ...
            {
                settings.SetSettingValue("PartOrderBy",0); 
            }

        }
        string defu_Order = "default";
        private async void LoadPart(Parts parts)
        {
        ...
        private async Task<ObservableCollection<TagsModel>> GetTags(int id)
        {
            ...
            try
            {
                ...
                string zh_result = await wc.GetResults(new Uri("http://api.bilibili.com/x/tag/hots?rid=" + id + "&type=0&jsonp=json?rnd=" + new Random().Next(1, 9999)));
        ...
        private async Task<ObservableCollection<DHModel>> GetVideos(int id, string orderBy, int Num, string tag)
        {
            ...
            try
            {
                ...
                uri = string.Format("http://api.bilibili.com/list?_device=android&appkey={0}&build=422000&mobi_app=android&order={3}&page={1}&pagesize=20&platform=android&tid={2}", ApiHelper._appKey_Android, Num, id, orderBy);
                if (tag.Length != 0 && tag != "全部")
                {
                    uri += "&tag=" + Uri.EscapeDataString(tag);
                }
        ...
        private async Task<List<DHModel>> GetBanner(int Id)
        {
            ...
            try
            {
                ...
                string results = await wc.GetResultsUTF8Encode(new Uri("http://app.bilibili.com/api/region2/" + Id + ".json"));
                DHModel model = JsonConvert.DeserializeObject<DHModel>(results);
        ...
        private async Task<List<DHModel>> GetTuiJianDT(int Id)
        {
            ...
            try
            {
                ...
                string results = await wc.GetResults(new Uri("http://www.bilibili.com/index/ding/" + Id + ".json?rnd=" + new Random().Next(1, 9999)));
                DHModel model = JsonConvert.DeserializeObject<DHModel>(results);
        ...
        private void pivot_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            foreach (var item in pivot.ItemsSource as List<PartModel>)
            {
            ...
            {
                case PartOrderBy.hot:
                    btn_Play.IsChecked = true;
                    break;
                case PartOrderBy.damku:
                    btn_Danmaku.IsChecked = true;
                    break;
                case PartOrderBy.review:
                    btn_Comment.IsChecked = true;
                    break;
                case PartOrderBy.stow:
                    btn_Sc.IsChecked = true;
                    break;
                case PartOrderBy.Default:
        ...
        private void btn_New_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                case 0:
                    (pivot.SelectedItem as PartModel).orderBy = PartOrderBy.Default;
                    break;
                case 1:
                    (pivot.SelectedItem as PartModel).orderBy = PartOrderBy.damku;
                    break;
                case 2:
                    (pivot.SelectedItem as PartModel).orderBy = PartOrderBy.hot;
                    break;
                case 3:
                    (pivot.SelectedItem as PartModel).orderBy = PartOrderBy.review;
                    break;
                case 4:
                    (pivot.SelectedItem as PartModel).orderBy = PartOrderBy.stow;
                    break;
                default:
                    break;
            }
